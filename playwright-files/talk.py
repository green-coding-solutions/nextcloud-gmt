...
def talk(playwright: Playwright, url: str, browser_name: str) -> None:
    ...
    contexts = [browser.new_context(ignore_https_errors=True, viewport={'width': 1280, 'height': 720}) for browser in browsers]
    ...
    for i, sender in enumerate(pages):
        log_note(f"talk_message={int(MESSAGE_CHARS/TALK_INVITEE_COUNT)}") # Per Character Per User
