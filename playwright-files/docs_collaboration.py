...
def collaborate(playwright: Playwright, browser_name: str) -> None:
    ...
    if browser_name == "firefox":
        browser_two = playwright.firefox.launch(headless=False)
    else:
        browser_two = playwright.chromium.launch(headless=False, args=['--disable-gpu', '--disable-software-rasterizer', '--ozone-platform=wayland'])
    ...
    try:
        ...
        for x in range(1, TEXT_CHUNKS+1):
            log_note(f"document_editing={int(CHARACTER_AMOUNT_PER_CHUNK/PARTICIPANTS)}") # Character per Participant
